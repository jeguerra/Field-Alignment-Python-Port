function [qtx,qty,qq,errl] = FA2DC4V2(Xb1,Y1,H1,Xu1,Yu1,Hu1,msk,fltr, niter, graphics,wt,lscale,mode,debug)
%% Description:
...
szx = size(Xb1); nx = szx(1); %%% We just assume it is square.
pblmr = 1:nx;
 Xb  = zeros(nx); H = zeros(szx);
...
for i = 1:size(Xb1,3)
    ...
    mxb(i) = min(Xb(:)); dxb(i) = max(Xb(:)) -mxb(i);
    Xb = (Xb - mxb(i))./dxb(i);
    Y = (Y - mxb(i))./dxb(i);
    Xb1(:,:,i) = Xb;
    Y1(:,:,i) = Y;
...
iter = 0; err =[];
errl = inf; errv = inf; derrl = -inf;
while((iter<niter) && errl>1e-4 && errv> 1e-5 && (derrl)<1e+1),
    ...
    ssx1 = msk.*ssx(pblmr,pblmr); ssy1  =msk.*ssy(pblmr,pblmr);
    [aa,bb] = meshgrid(1:length(ssx1),1:length(ssx1));
    
    % If user wants graphics
    if (graphics && (rem(iter,100)==0))
        hf=figure(3);set(hf,'WindowStyle','docked');
        subplot(121);   quiver(aa(1:2:length(ssx1),1:2:length(ssx1)),...
            bb(1:2:length(ssx1),1:2:length(ssx1)),...
            ssx1(1:2:length(ssx1),1:2:length(ssx1)),...
            ssy1(1:2:length(ssx1),1:2:length(ssx1)),1.2,'k');
        axis('square');axis('off');title('Instaneous Displacement');
        subplot(122);   quiver(aa(1:2:length(ssx1),1:2:length(ssx1)),...
            bb(1:2:length(ssx1),1:2:length(ssx1)),...
            qtx(1:2:length(ssx1),1:2:length(ssx1)),...
            qty(1:2:length(ssx1),1:2:length(ssx1)),1.2,'k');
        axis('square');axis('off');title('Total Displacement');
        
        
    end
    
    %% p - q, interpolate.
    qx = aa - ssx1;        qy = bb - ssy1;
    qtx = qtx + ssx1; qty = qty+ssy1; %% You can also interpolate with total disp.
    ...
    for i = 1:size(Xb1,3)
        ...
        XXb1 = interp2(bdx,bdy,XXb1,bd+qx,bd+qy,'bicubic');
      %  XXb1 = bicutest(bdx,bdy,XXb1,bd+qx,bd+qy);
        ...
        %% Calc forcing here
        dq = H(:,:,i).*(Xb1(:,:,i) - Y1(:,:,i));
        dq(1,1:end) = 0; dq(end,1:end) = 0; dq (1:end,end) = 0; dq(1:end,1) = 0;
        [dXbx,dXby] = gradient(Xb1(:,:,i));
        t1 = t1-dXbx.*dq;            t2 = t2-dXby.*dq;
        dqe = median([dqe;abs(dq(H(:,:,i)>0))]);
      
        if (graphics  && (rem(iter,100)==1))
            hf=figure(2);set(hf,'WindowStyle','docked');
            subplot(size(Xb1,3),3,(i-1)*3+1);contourf(Xb1(pblmr,pblmr,i),10);axis('square');axis('off');
            subplot(size(Xb1,3),3,(i-1)*3+2); contourf(Y1(pblmr,pblmr,i),10);axis('square'); axis('off');
            if ((sum(sum(H(:,:,i)))<=0.25*numel(dq)) && (graphics<2))
                [Ry,Cy] = find(H(pblmr,pblmr,i)==1); 
                hold on; plot(Cy,Ry,'.w'); hold off;  
            end

            subplot(size(Xb1,3),3,(i-1)*3+3);contourf(Xb1(pblmr,pblmr,i)-Y1(pblmr,pblmr,i));colorbar;axis('square');axis('off');
            drawnow;
        end
    end
if (~isempty(Hu1))    
    ...
    Ub1 = interp2(bdx,bdy,UUb1,bd+qx,bd+qy,'bicubic');
   
    VVb1 = Xu1(pblmr,pblmr,2);
    bd = min(10,size(VVb1,1));
    VVb1 = Border(VVb1,bd); 
    [bdx,bdy]=meshgrid(1:size(VVb1,2),1:size(VVb1,1));
    Vb1 = interp2(bdx,bdy,VVb1,bd+qx,bd+qy,'bicubic');
   ...
   [dUbx,dUby] = gradient(Xu1(:,:,1));  
    tu1 = -1.*dUbx.*dqu;    tu2 = -1.*dUby.*dqu;
  dqv = Hu.*(Xu1(:,:,2) - Yu1(:,:,2));    
  [dVbx,dVby] = gradient(Xu1(:,:,2));  
  tv1 = -1.*dVbx.*dqv;    tv2 = -1.*dVby.*dqv;
    ...
    fFx = (fftshift(fft2(t1,lscale,lscale)));    fFy = (fftshift(fft2(t2,lscale,lscale)));
    % Avoid singularity
    fFx(lscale/2+1,lscale/2+1) = 0;     fFy(lscale/2+1,lscale/2+1) = 0;
    
    %Apply deformation filters
    fac1 = w2.*n.^2+w1.*(n.^mode+m.^mode);
    fac2 = w2.*m.*n;
    fac4=  w2.*m.^2+w1.*(m.^mode+n.^mode);
    xpy = (fac1-fac2); ypx = (fac4-fac2);  zz = (fac1.*fac4-fac2.*fac2)*wt/lscale;
    ...
    % criterion.
    errv = max(max(max(abs(ux))), max(max(abs(uy))));

    % Logging here.
    if (iter >2)
        errl = err(length(err));
        derrl = errl - err(length(err) -1);
        if (debug && (rem(iter,100)==1))
            ss=sprintf('%d %f %f %f',iter, errv, errl, derrl);
            disp(ss);
        end
    end;
